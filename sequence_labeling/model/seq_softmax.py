...
__all__ = ["SequenceSoftmax"]

class SequenceSoftmax(BaseModel):
    """sequence softmax model"""
    def __init__(self,
                 logger,
                 hyperparams,
                 data_pipeline,
                 mode="train",
                 scope="seq_softmax"):
        """initialize sequence softmax model"""
        super(SequenceSoftmax, self).__init__(logger=logger, hyperparams=hyperparams,
    ...
    def build(self,
              sess):
        """build saved model for sequence softmax model"""
    ...
    def save(self,
             ...
             global_step,
             save_mode):
        """save checkpoint for sequence softmax model"""
        if save_mode == "debug":
    ...
    def restore(self,
                ...
                ckpt_file,
                ckpt_type):
        """restore sequence softmax model from checkpoint"""
    ...
    def get_latest_ckpt(self,
                        ckpt_type):
        """get the latest checkpoint for sequence softmax model"""
        if ckpt_type == "debug":
    ...
    def get_ckpt_list(self,
                      ckpt_type):
        """get checkpoint list for sequence softmax model"""
        if ckpt_type == "debug":
